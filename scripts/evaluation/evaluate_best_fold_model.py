...
# 模型文件（最佳折的模型）
MODEL_PATH = os.path.join(BASE_DIR, 'pointnet_regression_model_kfold_fold1_best.pth')
...
def evaluate_best_fold_model():
    """评估K折最佳折的模型（折1）"""
    print("\n" + "="*70)
    print("评估K折最佳折的模型（折1，验证损失0.000136）")
    print("="*70)
    print("⚠️  这是真正的最佳模型，没有被重新训练覆盖")
    ...
    print(f"\n📊 总体性能:")
    print(f"   所有坐标的RMSE: {overall_rmse:.4f} mm")
    print(f"   所有坐标的MAE: {overall_mae:.4f} mm")
    print(f"   所有点的3D RMSE: {overall_rmse_3d:.4f} mm")
    print(f"   所有点的3D MAE: {overall_mae_3d:.4f} mm")
    print(f"\n📊 平均精度（基于欧氏距离）:")
    for threshold in thresholds:
    ...
    for point_idx in range(9):
        print(f"\n📍 地标点 {point_idx + 1}/9: {landmark_names[point_idx]}")
        print(f"   {'-'*60}")
        
        # 3D误差
        print(f"   3D误差:")
        print(f"     RMSE: {point_rmse_3d[point_idx]:.4f} mm")
        print(f"     MAE: {point_mae_3d[point_idx]:.4f} mm")
        print(f"     最大误差: {np.max(point_errors_3d[:, point_idx]):.4f} mm")
        print(f"     最小误差: {np.min(point_errors_3d[:, point_idx]):.4f} mm")
        print(f"     中位数误差: {np.median(point_errors_3d[:, point_idx]):.4f} mm")
        print(f"     标准差: {np.std(point_errors_3d[:, point_idx]):.4f} mm")
        
        # X, Y, Z各轴的误差
        print(f"   各轴误差:")
        print(f"     X轴 - RMSE: {rmse_x[point_idx]:.4f} mm, MAE: {mae_x[point_idx]:.4f} mm")
        print(f"     Y轴 - RMSE: {rmse_y[point_idx]:.4f} mm, MAE: {mae_y[point_idx]:.4f} mm")
        print(f"     Z轴 - RMSE: {rmse_z[point_idx]:.4f} mm, MAE: {mae_z[point_idx]:.4f} mm")
        
        # 误差分布和精度（基于欧氏距离）
        errors_point = point_errors_3d[:, point_idx]
        precision_1mm = np.sum(errors_point < 1) / len(errors_point) * 100
        precision_2mm = np.sum(errors_point < 2) / len(errors_point) * 100
        precision_5mm = np.sum(errors_point < 5) / len(errors_point) * 100
        precision_10mm = np.sum(errors_point < 10) / len(errors_point) * 100
        
        print(f"   误差分布（基于欧氏距离）:")
        print(f"     < 1mm: {np.sum(errors_point < 1):3d} 个样本 ({precision_1mm:.1f}%)")
        print(f"     < 2mm: {np.sum(errors_point < 2):3d} 个样本 ({precision_2mm:.1f}%)")
        print(f"     < 5mm: {np.sum(errors_point < 5):3d} 个样本 ({precision_5mm:.1f}%)")
        print(f"     < 10mm: {np.sum(errors_point < 10):3d} 个样本 ({precision_10mm:.1f}%)")
        print(f"     >= 10mm: {np.sum(errors_point >= 10):3d} 个样本 ({100-precision_10mm:.1f}%)")
        
        print(f"   精度（基于欧氏距离）:")
        print(f"     精度 @ 1mm: {precision_1mm:.2f}%")
        print(f"     精度 @ 2mm: {precision_2mm:.2f}%")
        print(f"     精度 @ 5mm: {precision_5mm:.2f}%")
        print(f"     精度 @ 10mm: {precision_10mm:.2f}%")
        
        # 与目标对比
        if point_rmse_3d[point_idx] < 2:
            status = "✅ 达到目标 (< 2mm)"
        elif point_rmse_3d[point_idx] < 5:
            status = "⚠️  接近目标 (< 5mm)"
        else:
            status = "❌ 需要改进 (>= 5mm)"
        print(f"   状态: {status}")
    ...
    # 汇总表
    print(f"\n" + "="*70)
    print("汇总表：所有9个地标点的性能")
    print("="*70)
    print(f"{'地标点':<15} {'3D RMSE':<12} {'3D MAE':<12} {'精度@2mm':<12} {'精度@5mm':<12} {'X RMSE':<12} {'Y RMSE':<12} {'Z RMSE':<12} {'状态':<15}")
    print("-" * 110)
    for point_idx in range(9):
        precision_2mm = np.sum(point_errors_3d[:, point_idx] < 2) / len(point_errors_3d) * 100
        precision_5mm = np.sum(point_errors_3d[:, point_idx] < 5) / len(point_errors_3d) * 100
        if point_rmse_3d[point_idx] < 2:
            status = "✅ 达标"
        elif point_rmse_3d[point_idx] < 5:
            status = "⚠️  接近"
            ...
            status = "❌ 需改进"
        print(f"{landmark_names[point_idx]:<15} {point_rmse_3d[point_idx]:<12.4f} {point_mae_3d[point_idx]:<12.4f} "
              f"{precision_2mm:<12.2f} {precision_5mm:<12.2f} "
              f"{rmse_x[point_idx]:<12.4f} {rmse_y[point_idx]:<12.4f} {rmse_z[point_idx]:<12.4f} {status:<15}")
    
    # 保存结果
    results = {
        'model_info': {
            'model_name': 'K折最佳折模型（折1）',
            'model_file': 'pointnet_regression_model_kfold_fold1_best.pth',
            'validation_loss': 0.000136,
            'note': '这是真正的最佳模型，没有被重新训练覆盖'
        },
        'overall': {
    ...
    results_file = os.path.join(BASE_DIR, 'best_fold_model_evaluation_results.json')
