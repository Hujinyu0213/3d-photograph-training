...
def main():
    print("="*60)
    print("🧪 增强+FPS模型 vs 旧K折模型（无增强）测试集对比")
    print("="*60)
    
    aug_fps_model = os.path.join(ROOT_DIR, "models", "pointnet_regression_model_full_aug_fps_best.pth")
    aug_results = evaluate_model(aug_fps_model, "单次训练增强+FPS模型")
    
    old_kfold_model = os.path.join(ROOT_DIR, "models", "pointnet_regression_model_kfold_best.pth")
    if os.path.exists(old_kfold_model):
        kfold_results = evaluate_model(old_kfold_model, "旧K折模型（无增强）")
    else:
        kfold_results = None
        print("\n⚠️  旧K折模型文件不存在")
    ...
    print(f"\n整体指标对比 (毫米):")
    print(f"{'指标':<20} {'增强+FPS':>15} {'旧K折':>15} {'改进':>10}")
    print("-"*60)
    ...
    print(f"\n每个地标点3D误差对比 (RMSE, mm):")
    print(f"{'地标点':<15} {'增强+FPS':>12} {'旧K折':>12} {'改进':>10}")
    print("-"*60)
    ...
    output = {
        ...
        'old_kfold_model': kfold_results
    }
    
    output_path = os.path.join(ROOT_DIR, "results", "test_comparison_aug_fps_vs_old_kfold.json")
