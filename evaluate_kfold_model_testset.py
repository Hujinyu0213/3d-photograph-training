"""
在K折交叉验证的测试集上评估模型，详细分析所有9个地标点的性能
...
# 模型文件
MODEL_PATH = os.path.join(BASE_DIR, 'pointnet_regression_model_kfold_best.pth')
...
# 评估函数
# =========================================================
def evaluate_kfold_model():
    """在K折交叉验证的测试集上评估模型"""
    print("\n" + "="*70)
    print("在K折交叉验证测试集上评估模型")
    print("="*70)
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader, desc="评估")):
            ...
            # 获取对应的尺度和质心
            # test_dataset.indices 是原始数据集的索引
    ...
    results_file = os.path.join(BASE_DIR, 'kfold_test_evaluation_results.json')
