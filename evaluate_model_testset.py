...
def load_data():
    ...
    return torch.from_numpy(X_np), torch.from_numpy(Y_np), scales_list
...
def evaluate_model():
    ...
    print(f"   训练+验证集: {len(train_val_dataset)} 个样本")
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(tqdm(test_loader, desc="评估")):
            ...
            # 获取对应的尺度
            batch_indices = test_dataset.indices[batch_idx * 8:(batch_idx + 1) * 8]
            batch_scales = [scales_list[i] for i in batch_indices[:len(pred_np)]]
            all_scales.extend(batch_scales)
    ...
    for point_idx in range(9):
        ...
        # 3D误差
        print(f"   3D误差:")
        print(f"     RMSE: {point_rmse_3d[point_idx]:.4f}")
        print(f"     MAE: {point_mae_3d[point_idx]:.4f}")
        print(f"     最大误差: {np.max(point_errors_3d[:, point_idx]):.4f}")
        print(f"     最小误差: {np.min(point_errors_3d[:, point_idx]):.4f}")
        print(f"     中位数误差: {np.median(point_errors_3d[:, point_idx]):.4f}")
        print(f"     标准差: {np.std(point_errors_3d[:, point_idx]):.4f}")
        
        # X, Y, Z各轴的误差
        print(f"   各轴误差:")
        print(f"     X轴 - RMSE: {rmse_x[point_idx]:.4f}, MAE: {mae_x[point_idx]:.4f}")
        print(f"     Y轴 - RMSE: {rmse_y[point_idx]:.4f}, MAE: {mae_y[point_idx]:.4f}")
        print(f"     Z轴 - RMSE: {rmse_z[point_idx]:.4f}, MAE: {mae_z[point_idx]:.4f}")
    ...
    print("="*70)
    print(f"{'地标点':<8} {'3D RMSE':<12} {'3D MAE':<12} {'精度@2mm':<12} {'精度@5mm':<12} {'X RMSE':<12} {'Y RMSE':<12} {'Z RMSE':<12} {'状态':<15}")
    print("-" * 110)
    for point_idx in range(9):
        ...
        if point_rmse_3d[point_idx] < 2:
            ...
            status = "❌ 需改进"
        print(f"点 {point_idx+1:<4} {point_rmse_3d[point_idx]:<12.4f} {point_mae_3d[point_idx]:<12.4f} "
    ...
    for point_idx in range(9):
        ...
        point_results = {
            ...
            # 保留旧字段以兼容
            'pct_under_1mm': float(np.sum(errors_point < 1) / len(errors_point) * 100),
            'pct_under_2mm': float(np.sum(errors_point < 2) / len(errors_point) * 100),
            'pct_under_5mm': float(np.sum(errors_point < 5) / len(errors_point) * 100)
        }
        results['points'].append(point_results)
    
    results_file = os.path.join(BASE_DIR, 'test_evaluation_results.json')
